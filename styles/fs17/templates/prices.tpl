		...
		<table class="table table-hover table-bordered table-striped" id="allPrices" width="100%">
			...
			<tbody>
				...
				<tr>
					...
					<td class="text-right" width="200px">{if
						isset($fillTypeData.locations.$location)}{$fillTypeData.locations.$location.price|number_format:0:",":"."} {if $fillTypeData.priceTrend == 1} <span
						class="glyphicon glyphicon-triangle-top text-success" aria-hidden="true"></span>{elseif $fillTypeData.priceTrend == -1} <span
						class="glyphicon glyphicon-triangle-bottom text-danger" aria-hidden="true"></span>{else} <span class="glyphicon glyphicon-arrow-down"
						style="visibility: hidden"><span>{/if}{else}&nbsp;{/if} 

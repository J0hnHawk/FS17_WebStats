...
if ($_SERVER ['REQUEST_METHOD'] == 'POST') {
	...
	$options ['production'] ['hideNotUsed'] = filter_var ( GetParam ( 'hideNotUsed', 'P', 0 ), FILTER_VALIDATE_BOOLEAN );
...
foreach ( $plants as $plantName => $plant ) {
	...
	if ($options ['production'] ['hideNotUsed']) {
		$sumFillLevel = 0;
		foreach ( $plant ['input'] as $fillType => $fillTypeData ) {
			$sumFillLevel += $fillTypeData ['fillLevel'];
		}
		foreach ( $plant ['output'] as $fillType => $fillTypeData ) {
			$sumFillLevel += $fillTypeData ['fillLevel'];
		}
		if ($sumFillLevel == 0) {
			unset ( $plants [$plantName] );
